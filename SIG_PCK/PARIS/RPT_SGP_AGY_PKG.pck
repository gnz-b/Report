...
----CURR-YTD-FYP-B4-SOC precent the ADJ_TYD_RYP
    ...
    INSERT INTO AIA_PARIS 
      ...
      coalesce(incentive.GENERICNUMBER1, 0), incentive.VALUE
      FROM CS_MEASUREMENT mea
      INNER JOIN CS_PERIOD period ON period.PERIODSEQ = mea.PERIODSEQ AND period.REMOVEDATE > SYSDATE
      INNER JOIN CS_PAYEE pay ON mea.PAYEESEQ = pay.PAYEESEQ AND pay.REMOVEDATE > period.ENDDATE AND period.ENDDATE BETWEEN pay.EFFECTIVESTARTDATE AND pay.EFFECTIVEENDDATE - 1
      INNER JOIN CS_POSITION pos ON pos.PAYEESEQ = mea.PAYEESEQ AND pos.REMOVEDATE > SYSDATE AND period.ENDDATE BETWEEN pos.EFFECTIVESTARTDATE AND pos.EFFECTIVEENDDATE - 1
	    ...
	    TXT_LEADER_CODE, TXT_LEADER_NAME,TXT_LEADER_TITLE, DTE_AGENCY_DISSOLVED,
		  ...
		  TXT_MIN_RYP,
			...
			SELECT VL_CYCLE_MONTH, VL_CYCLE_YEAR,
      			 ...
      			 END,
      			 CASE 
      			   WHEN coalesce(permea.VALUE,0) > coalesce(fvperparis.VALUE, 0) THEN
      			 	  'Y'
      			   ELSE
      			 		'N'
      			 END,
			...
			INNER JOIN CS_POSITION POS 
								 ON POS.NAME = ('SGY' || paris.TXT_DISTRICT_CODE)
			...
			LEFT  JOIN CS_MEASUREMENT permea ON permea.POSITIONSEQ = POS.RULEELEMENTOWNERSEQ 
				         ...
				         AND permea.NAME = 'PM_District_PA_Persistency'
      INNER JOIN CS_FIXEDVALUE fvperparis ON fvperparis.NAME = 'FV_PARIS_PERSISTENCY' AND fvperparis.REMOVEDATE > SYSDATE AND V_PERIODENDDATE BETWEEN fvperparis.EFFECTIVESTARTDATE AND fvperparis.EFFECTIVEENDDATE
			LEFT  JOIN CS_MEASUREMENT minmea 
						ON   minmea.NAME = 'SM_DM_PARIS_RYP_YEAR_SG'
						AND  minmea.POSITIONSEQ = leader.RULEELEMENTOWNERSEQ
						AND  minmea.PERIODSEQ = V_PERIODSEQ
		  LEFT  JOIN CS_INCENTIVE incentive 
		  	    ON   incentive.NAME = 'I_PARIS_DM_SG'
